...
<div class="card">
   ...
   <form id="formid" class="row g-3" method="POST"  name="formid" action="forms/sqlupdates.php?action=qualityanalysis&requestorname=<?php echo $fullname;?>">

   <?php
   
   $topic = "qualityanalysis";
   ...
   ?>
          ...
          <div class="col-md-6">
                ...
                <option selected></option>
                           <?php
                              $commontrainingtopic = "SELECT available_topic FROM [dbo].[tbl_topicsetting] WHERE month ='$monthtoday' AND category = 'Quality Analysis' ORDER BY available_topic ASC";
...
<form>
        ...
        <tbody>
        <?php
               $selectcommon = "SELECT * FROM [dbo].[tbl_topic_LIST_qualityanalysis_MAINREQUEST] where Section = '$section'";
...
document.getElementById('btn-submit2').addEventListener('click', function() {
    ...
    Swal.fire({
        title: 'Are you sure?',
        html: 'Do you want to enlist to Elearning the following selected employees?<br>' + selectedEmployees.join('<br>'),
        icon: 'question',
        showCancelButton: true,
        confirmButtonText: 'Yes',
        cancelButtonText: 'No',
        ...
        if (result.isConfirmed) {
            // If user clicks "Yes", submit the form and pass the selected values to another form
            // Here you can pass the selected values to another form or perform any other action you need
          // Convert array to string with comma as delimiter
          var selectedEmployeesString = selectedEmployees.join(',');
          var selectedTopic = document.getElementById('topic').value;
          ...
          var enddate = document.getElementById('enddate').value;
            ...
            window.location.href = 'mainprocess.php?function=UploadAttendees&topic=' + encodeURIComponent(selectedTopic) +'&startdate='+  encodeURIComponent(startdate)+'&enddate='+  encodeURIComponent(enddate)+'&selectedrows=' + selectedEmployeesString;
    ...
    if (count <= 0) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          window.location.href = 'forms/sqlupdates.php?action=qualityanalysisRemove&selectedItemId=' + vals.join(',') +
