        ...
        <div class="col-md-3 offset-md-9">
            <fieldset>
              <legend> 
                  ...
                  <a id="followup_link" href="email/FollowupEmail.php?title=" style="text-decoration: none;">
            ...
            <select class="form-control" id="comboBox" >
                ...
                <?php
                    ...
                    $sql_fetch = "SELECT DISTINCT(Title) FROM [dbo].[tbl_elearningstatus]";
                    $stmt = sqlsrv_query($conn, $sql_fetch);
                    
                    while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
                        $title = $row['Title'];
                        // Escape single quotes in the title to prevent issues with attribute values
                        $escapedTitle = htmlspecialchars($title, ENT_QUOTES);
                        echo "<option value='" . $escapedTitle . "' data-title='" . $escapedTitle . "'>" . $escapedTitle . "</option>";
                    }

                    // Free statement resources to avoid exhausting memory
...
<div class="card">
    <div class="card-body">
        <h5 class="card-title"></h5>
                                                    ...
                                                    <?php
                // Fetch Titles from tbl_topic_code
                // Query to fetch target counts and finished counts
                $query = "
...
<table>
        <thead>
            <tr>
                ...
                <th colspan="<?php echo count($titles); ?>" style='color:#3AB050; text-align:center;'>Target</th>
                <th colspan="<?php echo count($titles); ?>" style='color:#3AB050; text-align:center;'>Done</th>
                <th colspan="<?php echo count($titles); ?>" style='color:#3AB050; text-align:center;'>Percentage</th>
            </tr>
            <tr>
                <?php 
                // Dynamically create headers for Topic_Codes for Target Count
                foreach ($titles as $title) {
                    $topicCode = isset($topicCodes[$title]) ? $topicCodes[$title] : 'N/A';
                    echo "<th style='color:#3AB050'>{$topicCode} Target Count</th>";
                }
                // Dynamically create headers for Topic_Codes for Finished Count
                foreach ($titles as $title) {
                    $topicCode = isset($topicCodes[$title]) ? $topicCodes[$title] : 'N/A';
                    echo "<th style='color:#3AB050'>{$topicCode} Finished Count</th>";
                }
                // Dynamically create headers for Topic_Codes for Percentage
                foreach ($titles as $title) {
                    $topicCode = isset($topicCodes[$title]) ? $topicCodes[$title] : 'N/A';
                    echo "<th style='color:#3AB050'>{$topicCode} Percentage</th>";
                }
                ?>
            </tr>
        </thead>
