...
<div class="card">
            ...
            <div class="card-body">
              <h5 class="card-title">PCBA - Hashira</h5>
   ...
   <form id="formid" class="row g-3" method="POST"  name="formid" action="forms/sqlupdates.php?action=PCBAprodhashira&requestorname=<?php echo $fullname;?>">

   <?php
   $topic = 'PCBAprodhashira';
   $monthtoday = date('F Y');
   ...
   ?>
          ...
          <div class="col-md-6">
                ...
                <option value="" selected disabled>Select a topic</option>
                        <?php
                              $commontrainingtopic = "SELECT available_topic FROM [dbo].[tbl_topicsetting] WHERE month ='$monthtoday' AND category = 'Prod. Hashira' ORDER BY available_topic ASC";
...
<form>
        ...
        <tbody>
        <?php
               $selectcommon = "SELECT * FROM [dbo].[tbl_elearningstatus] where Section = 'PCBA'";
               $stmt3 = sqlsrv_query($conn,$selectcommon);
                              while($row = sqlsrv_fetch_array($stmt3, SQLSRV_FETCH_ASSOC)) {
                                  $id2 = $row['ID'];
                                  $employeeno2 = $row['EmployeeNumber'];
                                  $fullname2 = $row['Target_Employee'];
                                  $section2 = $row['Section'];
                                  $title = $row['Title'];
                                  $email2 = $row['Email'];
                                
                   
                                  $startdate2 = $row['Target_Date'];
                                  $enddate2 = $row['End_Date'];
                                  ...
                                  '<tr>
                                    ...
                                    <td style=" font-size: 12px;">'  . $row['Title'] .'</td>
                                      <td style=" font-size: 12px;">'  . $row['EmployeeNumber'] .'</td>
                                      <td style=" font-size: 12px;">'  . $row['Target_Employee'] .'</td>
                                      <td style=" font-size: 12px;">'  . $row['Target_Date'] .'</td>
...
<script>
  ...
  if (count <= 0) {
    ...
    Swal.fire({
      ...
      text: 'Control Number: ' + (vals.length > 0 ? vals.join(', ') : ''),
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
      ...
      if (result.isConfirmed) {
        ...
        window.location.href = 'forms/sqlupdates.php?action=PCBAprodhashiraRemove&selectedItemId=' + vals.join(',');
      }
    });
  }
}
