...
<div class="card">
   ...
   <form id="formid" class="row g-3" method="POST"  name="formid" action="forms/sqlupdates.php?action=companyfundamentals&requestorname=<?php echo $fullname;?>">

   <?php
   
   $topic = 'companyfundamentals';
   $monthtoday = date('F Y');
   ?>
          ...
          <div class="col-md-6">
                ...
                <select class="form-select"  onchange="selectComboBox()" id="topic" name="topic" aria-label="State" style="font-weight: bold; border-width:2px; border-style:solid;  padding: 10px; background-color: #FFF7DA; padding-top:25px;"required>
                <option value="" selected disabled>Select a topic</option>
                           <?php
                              $commontrainingtopic = "SELECT available_topic FROM [dbo].[tbl_topicsetting] WHERE month ='$monthtoday' AND category = 'Company Fundamentals' ORDER BY available_topic ASC";
    ...
    if (count <= 0) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          window.location.href = 'forms/sqlupdates.php?action=companyfundamentalsRemove&selectedItemId=' + vals.join(',') +
